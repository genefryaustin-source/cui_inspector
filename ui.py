...
def render_document_inspector():
    ...
    with colB:
        ...
        rs_name = st.selectbox("Ruleset", ruleset_names(), index=0)
        st.caption(RULESETS[rs_name]["description"])

        can_analyze = bool((st.session_state.get("last_text") or "").strip())
        if st.button("▶ Run Analysis", type="primary", disabled=not can_analyze):
